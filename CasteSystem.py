...
async def describe_me(interaction: discord.Interaction):
    tutorial_message = (
        ...
        f"You must assign a blood caste using one of these roles: {', '.join(ROLE_NAMES)}\n"
        "Use the command `/changeblood` to do so.\n"
        "This bot also gets the capital initials of your nickname, "
        "so if you have a name like **EctoBiologist**, it will be changed to **EB:** when chatting.\n"
        "And if you ever want to read this tutorial again, you can use the command `/tutorial`.\n"
        "--END OF TUTORIAL--"
    )
